...
function App() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('openguide', handler)
    return () => window.removeEventListener('openguide', handler)
  }, [])
  ...
  return (
    <ToastProvider>
      <div className="app">
          ...
          <div className={`nav-links ${menuOpen ? 'open' : ''}`}>
            {token ? (
              <>
                ...
                <button className="btn btn-outline btn-sm" onClick={() => { setShowGuide(true); setMenuOpen(false) }}>❓ Guide</button>
        ...
        <InstallPrompt />
        <ReminderService token={token} enabled={!!token} />
        <HowItWorks show={showGuide} onClose={() => setShowGuide(false)} />
        {token && (
          <button className="guide-fab" onClick={() => setShowGuide(true)} title="How it works">
            ❓
          </button>
        )}
